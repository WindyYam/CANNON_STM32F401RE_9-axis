...
static imu_status_t imu_sensor_gyro_set_fs( float fullScale );
static imu_status_t imu_sensor_x_set_fs( float fullScale );
...
imu_status_t imu_sensor_set_data_rate(uint32_t* p_data_rate, uint8_t mode)
{
  ...
  /*acc and gyro odr */
  {
    ...
    if (sensor_selection & GYRO_ENABLE) {
      ...
      if (imu_sensor_config_gyro(new_odr, LSM6DS3_G_FS_500) != imu_status_ok) {
        return imu_status_fail;
      }
    }
...
void imu_sensor_read_data_from_fifo(void)
{
  ...
  uint16_t fifo_length = imu_sensor_get_fifo_datalength();
  printf("~~~%d\n", fifo_length);
...
uint16_t imu_sensor_get_fifo_datalength(void)
{
...
static imu_status_t imu_sensor_clear_fifo(void)
{
  ...
  fifo_data_number = imu_sensor_get_fifo_datalength();
